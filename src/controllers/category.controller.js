...
const getAllCategories = async (req, res) => {
  ...
  try {
    if (!categoryName) {
      const allCategories = await Category.findAll({ attributes: ['categoryID', 'categoryName'] })
      allCategories.length === 0
        ? res.status(404).json({ message: 'ERROR 404 - Not Found: get all Categories.' })
        : res.status(200).json(allCategories)
    } else {
      const category = await Category.findOne({ where: { categoryName }, attributes: ['id_category', 'categoryName'] })
      !category
        ? res.status(404).json({ message: 'ERROR 404 - Not Found: get all Categories.' })
...
const getCategoryByPK = async (req, res) => {
  ...
  try {
    const category = await Category.findByPk(categoryID, { attributes: ['categoryID', 'categoryName'] })
    !category
...
const deleteCategory = async (req, res) => {
  const { categoryID } = req.params
  try {
    const category = await Category.findByPk(categoryID, { attributes: ['categoryID'] })
